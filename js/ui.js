...
function renderFamiliars() {
  ...
  const placeholder = (familiarImages && familiarImages.default) || 'img/familiars/familiars.png';
    ...
    const libraryCount = fam.library?.length ?? 0;
    const stampCount = fam.collectibles?.stamps?.length ?? 0;
    const toyCount = fam.collectibles?.toys?.length ?? 0;
    const plantCount = fam.collectibles?.plants?.length ?? 0;
...
function showFamiliarDetail(familiarId) {
  ...
  if (familiar.library && (familiar.library?.length ?? 0) > 0) {
    libraryHtml += '<ul>';
  ...
  if (familiar.collectibles) {
    for (const category in familiar.collectibles) {
      ...
      if ((familiar.collectibles?.[category]?.length ?? 0) > 0) {
        collectiblesHtml += '<ul>';
