...
function startBattle(familiarId) {
  ...
  if (typeof playerFamiliar.currentHp !== "number") {
    playerFamiliar.currentHp = Number(playerFamiliar.hp) || 50;
  }
  battleState.playerFamiliar = { ...playerFamiliar, currentHp: playerFamiliar.currentHp, id: playerFamiliar.id };
  battleState.opponentFamiliar = { ...opponentFamiliar, currentHp: typeof opponentFamiliar.currentHp === "number" ? opponentFamiliar.currentHp : (Number(opponentFamiliar.hp) || 50) };
  battleState.log = [];
...
function useItemInBattle(itemId) {
  ...
  if (item.effect) {
    switch (item.effect.type) {
      case 'heal':
        if (typeof player.currentHp !== "number") {
          player.currentHp = Number(player.hp);
        }
        const healAmount = item.effect.amount === 'max' ? 
          (Number(player.hp) - Number(player.currentHp)) : 
          item.effect.amount;
        
        if (player.currentHp < Number(player.hp)) {
          player.currentHp = Math.min(Number(player.hp), player.currentHp + healAmount);
