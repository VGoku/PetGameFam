...
function startTraining(stat) {
    ...
    // Get UI elements
    const container = [...document.querySelectorAll('.training-option')]
        .find(el => el.querySelector(`button[onclick*="${stat}"]`));

    if (!container) {
        showNotification("Training UI not found.");
        return;
    }
    ...
    if (!button || !progressContainer || !progressBar) {
        showNotification("Training UI elements missing.");
        return;
    }
...
function initializeTrainingArea() {
    const familiar = getActiveFamiliar();
    const displayEl = document.getElementById('active-familiar-display');
    if (!familiar) {
        if (displayEl) displayEl.innerHTML = '<p>Select an active familiar to begin training!</p>';
        return;
    }
